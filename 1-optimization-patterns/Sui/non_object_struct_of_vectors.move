module move_gas_optimization::non_object_struct_of_vectors{
    ...
    public struct Parallel_Vectors has store, drop{
        vec1: vector<u64>,
        vec2: vector<u64>,
        vec3: vector<u64>
    }
    ...
    public entry fun non_obj_parallel_vectors_pushback_access(){
        ...
        while(i < length){
            ...
            let mut test_num = vector::borrow_mut(& mut item.vec1, i);     //grabbing mutable reference to vector element at i
    ...
    public entry fun non_obj_parallel_vectors_pushback_update(){
        ...
        while(i < length){
            //Updating vectors:
            let mut curr_num = vector::borrow_mut(&mut item.vec1, i);     //grabbing mutable reference to vector element at i
            *curr_num = 10;
